...
```{r echo=TRUE, message=TRUE, results='markup', collapse=TRUE}
r <- c(0.5, 1, 1.5, 2.0, 2.5)
area <- 2*pi*r^2
plot(r, area,xlab = 'Radius', ylab = 'Area',type = 'l', main = 'Radius vs Area plot')
```

echo: (True/False) This option allows us to display the source code in
the output. A value of 'True' shows the code and 'False hides the code.

results: This converts the text output to markup output format based on
the appropriate environments.

collapse: (True/False), to collapse the source and output into one block
when possible select True.
